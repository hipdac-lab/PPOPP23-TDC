...
extern "C" __global__ void default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ compute) {
  ...
  for (int ry_outer = 0; ry_outer < 3; ++ry_outer) {
    for (int rx_outer = 0; rx_outer < 3; ++rx_outer) {
      __syncthreads();
      pad_temp_shared[(((((int)threadIdx.z) * 14) + ((int)threadIdx.x)))] = (((((1 <= (((int)blockIdx.y) + ry_outer)) && ((((int)blockIdx.y) + ry_outer) < 15)) && (1 <= (((int)threadIdx.x) + rx_outer))) && ((((int)threadIdx.x) + rx_outer) < 15)) ? data[(((((((((int)threadIdx.z) * 196) + (((int)blockIdx.y) * 14)) + (ry_outer * 14)) + rx_outer) + ((int)threadIdx.x)) - 15))] : 0.000000e+00f);
      for (int ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner = 0; ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner < 3; ++ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner) {
        if (((((((int)threadIdx.x) * 3) + ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner) >> 5) + ((int)threadIdx.z)) < 32) {
          if ((((((int)threadIdx.z) * 32) + (((int)threadIdx.x) * 3)) + ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner) < 1024) {
            if (((((int)threadIdx.x) * 3) + ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner) < 32) {
              kernel_shared[((((((int)threadIdx.z) * 32) + (((int)threadIdx.x) * 3)) + ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner))] = kernel[((((((((int)threadIdx.z) * 288) + (((int)threadIdx.x) * 27)) + (ax0_ax1_fused_ax2_fused_ax3_fused_inner_inner_inner * 9)) + (ry_outer * 3)) + rx_outer))];
            }
          }
        }
      }
      ...
      for (int ax1 = 0; ax1 < 32; ++ax1) {
        pad_temp_shared_local[(ax1)] = pad_temp_shared[(((ax1 * 14) + ((int)threadIdx.x)))];
      }
