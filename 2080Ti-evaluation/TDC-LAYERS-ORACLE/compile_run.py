...
if __name__ == '__main__':
    output_path = '../../evaluation_outcome/2080Ti-layers-eval-oracle.csv'
    if os.path.exists(output_path):
        os.remove(output_path)
    Path(output_path).touch()
    writter = codecs.open(output_path, 'a', 'utf-8')
    writter.write('N,C,H,W,FFT(ms),WinoGradNonFuse(ms),GEMM(ms),TVM(ms),TDC Oracle(ms),Speedup VS FFT,Speedup VS WINO, Speedup VS GEMM, Speedup VS TVM\n')
    writter.close()
    shape_list = [(64,32,224,224),
                  ...
                  (192,160,7,7)]
    print('HDEADER')
    print('N,C,H,W,FFT(ms),WinoGradNonFuse(ms),GEMM(ms),TVM(ms),TDC Oracle(ms),Speedup VS FFT,Speedup VS WINO, Speedup VS GEMM, Speedup VS TVM')
