...
int main(void){
    ...
    chkerr(cudaFuncGetAttributes(&attr,conv2d));
    chkerr(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlks,conv2d,N,0));
    chkerr(cudaGetLastError());
    chkerr(cudaDeviceSynchronize());
    float occupancy = (numBlks * N * 1.0f)/1024;
    cout<<C<<","<<N<<","<<H<<","<<W<<","<<TH<<","<<TW<<","<<TC<<","<<occupancy<<","<<difference<<","<<time_tdc<<endl;
    return 0;
}